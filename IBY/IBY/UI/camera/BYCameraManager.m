...
@interface BYCameraManager ()

@property (nonatomic, strong) UIView *preview;

@end

@implementation BYCameraManager
...
- (void)addVideoInputFrontCamera:(BOOL)front {
    ...
    for (AVCaptureDevice *device in devices) {
        ...
        BYLog(@"Device name: %@", [device localizedName]);
        
        if ([device hasMediaType:AVMediaTypeVideo]) {
            
            if ([device position] == AVCaptureDevicePositionBack) {
                BYLog(@"Device position : back");
                backCamera = device;
                
            }  else {
                BYLog(@"Device position : front");
                frontCamera = device;
            }
        }
    }
    ...
    if (front) {
        ...
        if (!error) {
            if ([_session canAddInput:frontFacingCameraDeviceInput]) {
                ...
                BYLog(@"Couldn't add front facing video input");
            }
        }
    } else {
        ...
        if (!error) {
            if ([_session canAddInput:backFacingCameraDeviceInput]) {
                ...
                BYLog(@"Couldn't add back facing video input");
            }
        }
    }
}
...
- (void)takePicture:(DidCapturePhotoBlock)block {
    ...
    BYLog(@"about to request a capture from: %@", _stillImageOutput);

    [_stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        
        CFDictionaryRef exifAttachments = CMGetAttachment(imageDataSampleBuffer, kCGImagePropertyExifDictionary, NULL);
        if (exifAttachments) {
            BYLog(@"attachements: %@", exifAttachments);
        } else {
            BYLog(@"no attachments");
        }
        NSData *imageData = [AVCaptureStillImageOutput jpegStillImageNSDataRepresentation:imageDataSampleBuffer];
        UIImage *image = [[UIImage alloc] initWithData:imageData];
        BYLog(@"originImage:%@", [NSValue valueWithCGSize:image.size]);
        //        [SCCommon saveImageToPhotoAlbum:image];
        //CGFloat squareLength = SCREEN_APPLICATIONFRAME_SIZE.width;
...
//        CGSize size = CGSizeMake(SCREEN_BOUNDS_SIZE.width, SCREEN_BOUNDS_SIZE.height);
...
//        BYLog(@"scaledImage:%@", [NSValue valueWithCGSize:scaledImage.size]);
//        
//        CGRect cropFrame = CGRectMake((scaledImage.size.width - SCREEN_BOUNDS_SIZE.width) / 2, (scaledImage.size.height- SCREEN_BOUNDS_SIZE.height) / 2 , SCREEN_BOUNDS_SIZE.width, SCREEN_BOUNDS_SIZE.height);
//        BYLog(@"cropFrame:%@", [NSValue valueWithCGRect:cropFrame]);
        UIImage *croppedImage = image;//[scaledImage croppedImage:cropFrame];
//        BYLog(@"croppedImage:%@", [NSValue valueWithCGSize:croppedImage.size]);
        
        // We need to calculate the proper transformation to make the image upright.
        // We do it in 2 steps: Rotate if Left/Right/Down, and then flip if Mirrored.
        CGAffineTransform transform = CGAffineTransformIdentity;
        
        switch (croppedImage.imageOrientation) {
            case UIImageOrientationDown:
            case UIImageOrientationDownMirrored:
                transform = CGAffineTransformTranslate(transform, croppedImage.size.width, croppedImage.size.height);
                transform = CGAffineTransformRotate(transform, M_PI);
                break;
                
            case UIImageOrientationLeft:
            case UIImageOrientationLeftMirrored:
                transform = CGAffineTransformTranslate(transform, croppedImage.size.width, 0);
                transform = CGAffineTransformRotate(transform, M_PI_2);
                break;
                
            case UIImageOrientationRight:
            case UIImageOrientationRightMirrored:
                transform = CGAffineTransformTranslate(transform, 0, croppedImage.size.height);
                transform = CGAffineTransformRotate(transform, -M_PI_2);
                break;
            default:
                break;
        ...
        switch (croppedImage.imageOrientation) {
            ...
            case UIImageOrientationLeftMirrored:
            case UIImageOrientationRightMirrored:
                transform = CGAffineTransformTranslate(transform, croppedImage.size.height, 0);
                transform = CGAffineTransformScale(transform, -1, 1);
                break;
            default:
                break;
        ...
        // calculated above.
        CGContextRef ctx = CGBitmapContextCreate(NULL, croppedImage.size.width, croppedImage.size.height,
                                                 CGImageGetBitsPerComponent(croppedImage.CGImage), 0,
                                                 CGImageGetColorSpace(croppedImage.CGImage),
                                                 CGImageGetBitmapInfo(croppedImage.CGImage));
        ...
        switch (croppedImage.imageOrientation) {
            ...
            case UIImageOrientationRightMirrored:
                // Grr...
                CGContextDrawImage(ctx, CGRectMake(0,0,croppedImage.size.height,croppedImage.size.width), croppedImage.CGImage);
                break;
                
            default:
                CGContextDrawImage(ctx, CGRectMake(0,0,croppedImage.size.width,croppedImage.size.height), croppedImage.CGImage);
                break;
        }
        ...
        if (croppedImage.imageOrientation == UIImageOrientationUp){
            img = croppedImage;
        }
