...
- (void)layoutSubviews
{
    ...
    self.swipeableViewsCenterInitial = CGPointMake(
        self.bounds.size.width / 2 + self.swipeableViewsCenterInitial.x - self.swipeableViewsCenter.x,
        self.bounds.size.height / 2 + self.swipeableViewsCenterInitial.y - self.swipeableViewsCenter.y);
    self.swipeableViewsCenter = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2);
}
...
- (void)animateSwipeableViewsIfNeeded
{
    ...
    if (self.isRotationEnabled) {
        ...
        CGPoint rotationCenterOffset = {
            0, CGRectGetHeight(topSwipeableView.frame) * self.rotationRelativeYOffsetFromCenter
        };
...
- (void)handlePan:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded || recognizer.state == UIGestureRecognizerStateCancelled) {
        ...
        if (((signum(translation.x) == -1 && self.direction & ZLSwipeableViewDirectionLeft) || (signum(translation.x) == 1 && self.direction & ZLSwipeableViewDirectionRight) || (signum(translation.y) == -1 && self.direction & ZLSwipeableViewDirectionDown) || (signum(translation.y) == 1 && self.direction & ZLSwipeableViewDirectionUp)) && (ABS(translation.x) > self.relativeDisplacementThreshold * self.bounds.size.width // displacement
        ...
        // direction
        {
            ...
            CGVector direction = CGVectorMake(translation.x / translationMagnitude * scale,
...
- (void)swipeTopViewToLeft:(BOOL)left
{
    ...
    CGPoint location = CGPointMake(
        ...
        topSwipeableView.center.y * (1 + self.programaticSwipeRotationRelativeYOffsetFromCenter));
...
- (CGRect)defaultCollisionRect
{
    ...
    CGRect collisionRect = CGRectMake(-collisionSize.width / 2 + viewSize.width / 2,
